...
describe('email delivery idempotency', () => {
  ...
  test('covers all approved event types with concurrent deduplication and failure recording', async () => {
    jest.resetModules();
    const eventTypes = [
      'verification',
      'password-reset',
    ...
    const sendEmail = jest.fn(({ type }) => (
      type === 'password-reset'
        ? Promise.reject(new Error('provider unavailable'))
        : Promise.resolve({ messageId: `${type}-message` })
    ));
    const updateOne = jest.fn().mockResolvedValue({ acknowledged: true });

    jest.doMock('../src/config/db', () => ({ isMongoConnected: () => true }));
    jest.doMock('../src/models/EmailDelivery', () => ({
      findOneAndUpdate: jest.fn(async ({ eventKey }) => {
        if (claimed.has(eventKey)) {
          const error = new Error('duplicate key');
          error.code = 11000;
          throw error;
        }
        claimed.add(eventKey);
        return { _id: eventKey, status: 'sending' };
      }),
      findOne: jest.fn((query) => ({ lean: jest.fn().mockResolvedValue({ _id: query.eventKey, status: 'sending' }) })),
      updateOne,
    }));
    jest.doMock('../src/utils/email', () => ({ sendEmail }));

    const { sendTransactionalEmail } = require('../src/services/emailDeliveryService');
    const results = await Promise.all(eventTypes.map(async type => {
      const args = {
        type,
        eventId: `${type}-event`,
      ...
      return Promise.all([sendTransactionalEmail(args), sendTransactionalEmail(args)]);
    }));

    expect(sendEmail).toHaveBeenCalledTimes(eventTypes.length);
    expect(results.flat().filter(result => result.sent)).toHaveLength(eventTypes.length - 1);
    expect(results.flat().filter(result => result.duplicate)).toHaveLength(eventTypes.length);
    expect(results.flat().find(result => result.error)).toEqual(expect.objectContaining({ sent: false }));
    expect(updateOne).toHaveBeenCalled();
    expect(sendEmail.mock.calls.flat().join(' ')).not.toMatch(/token|password|secret/i);
  });
});
