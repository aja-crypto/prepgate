/**
 * CSE Cutoff Data Seed Script — LEGACY / FABRICATED DATA
 * 
 * !! WARNING: This script contains HARDCODED, UNVERIFIED, FABRICATED cutoff data.
 * !! DO NOT use this data for production predictions or user-facing displays.
 * !! The SEED_DATA values below are illustrative estimates, NOT verified source data.
 * !! Opening scores are NOT derived from closing scores in production.
 * !!
 * !! This script outputs to 'cse-cutoffs-legacy-fabricated.json' which is NOT used
 * !! by the application. The authoritative data lives in the MongoDB CcmtCutoff collection
 * !! and is regenerated by rebuild-static-cutoffs.js from verified DB rows only.
 * !!
 * !! NEVER calculate a fake opening score from closingScore.
 * !! If a real source does not provide openingScore, set it to null.
 * 
 * Run: node scripts/seedCseData.js (legacy only — not part of production workflow)
 */
...
async function seedDatabase() {
  ...
  const fs = require('fs');
  // WARNING: The SEED_DATA cutoffs above are illustrative/hardcoded, NOT verified.
  // Never overwrite the authoritative file (data/cse-cutoffs.json is regenerated from the
  // live DB by rebuild-static-cutoffs.js). Write to a clearly-labelled legacy file instead.
  const outputPath = path.join(__dirname, '../data/cse-cutoffs-legacy-fabricated.json');
  fs.writeFileSync(outputPath, JSON.stringify(jsonData, null, 2));
  console.log(`LEGACY FABRICATED JSON exported to: ${outputPath} (not used by the app)`);
