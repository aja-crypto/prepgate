...
async function executeSlot(slot, options = {}) {
  ...
  if (!SLOT_CRON[slot]) {
    return { generated, skippedDuplicates, skipped, slot };
  }
  ...
  for (const user of users) {
    try {
      ...
      console.error('[NotificationScheduler] user failed');
    }
  }

  console.log('[NotificationScheduler] slot executed', slot);
  console.log(`[NotificationScheduler] generated ${generated}`);
  console.log(`[NotificationScheduler] skipped ${skippedDuplicates} duplicates`);
  return { generated, skippedDuplicates, skipped, slot };
}
