...
function startScheduler() {
  ...
  // Notification scheduler — registered through the same lifecycle so
  // there is exactly one place that owns cron startup.
  try {
    startNotificationScheduler();
  } catch (err) {
    console.error('[cronJobs] notification scheduler failed to start:', err.message);
  }
