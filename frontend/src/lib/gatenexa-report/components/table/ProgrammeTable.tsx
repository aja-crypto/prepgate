...
export interface ProgrammeTableProps {
  ...
  noteEntityLabel?: string;
}

export const ProgrammeTable: React.FC<ProgrammeTableProps> = ({ data, noteEntityLabel }) => {
  const entityLabel = noteEntityLabel || data.title.toLowerCase();
  return (
    <View>
      <SectionTitle
      ...
      {data.remainingCount > 0 && (
        <View style={styles.note}>
          <Text style={styles.noteText}>
            + {data.remainingCount} additional {entityLabel} ({data.totalEligible} total) {data.csvNote}
          </Text>
        </View>
      )}
    </View>
