...
const MOCK_STORAGE_KEY = (id) => `gate2027_mock_progress_${id}`;
...
export default function MockTestRunner({ sessionId, onComplete, onCancel }) {
  ...
  // Load session + restore persisted answers
  useEffect(() => {
    (async () => {
      try {
        ...
        setSession(data);
        const duration = (data.config?.durationMinutes || 60) * 60;

        // Restore persisted progress
        try {
          const saved = JSON.parse(localStorage.getItem(MOCK_STORAGE_KEY(sessionId)));
          if (saved && saved.sessionId === sessionId) {
            setAnswers(saved.answers || {});
            setQuestionTimes(saved.questionTimes || {});
            setCurrent(saved.current || 0);
            const elapsed = Math.floor((Date.now() - saved.savedAt) / 1000);
            setTimeLeft(Math.max(0, duration - (saved.elapsed || 0) - elapsed));
            return;
          }
        } catch { /* fresh start */ }
  ...
  // Persist answers to localStorage every 30s + on answer change
  useEffect(() => {
    if (!session || results || !sessionId) return;
    const elapsed = ((session.config?.durationMinutes || 60) * 60) - timeLeft;
    const payload = {
      sessionId,
      answers,
      questionTimes,
    ...
    localStorage.setItem(MOCK_STORAGE_KEY(sessionId), JSON.stringify(payload));
  }, [answers, questionTimes, current, timeLeft, session, results, sessionId]);
  ...
  const handleSubmit = async (auto = false) => {
    ...
    try {
      ...
      const curTimes = questionTimesRef.current;
      const qs = curSession?.questions || [];
      const payload = qs.map((item) => {
        const id = item.pyq._id;
        const sel = curAnswers[id];
        return {
          pyqId: id,
          selectedAnswer: sel || null,
          timeTaken: curTimes[id] || 0,
          skipped: !sel,
        };
      });
      ...
      localStorage.removeItem(MOCK_STORAGE_KEY(sessionId));
  ...
  if (!pyq) return <div className="text-sm text-text3 py-12 text-center">Question unavailable</div>;
