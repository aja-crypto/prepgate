...
const MOCK_WEAK_TOPICS = [
  { subject: 'Operating Systems', topic: 'Process Synchronization', score: 34, reason: 'Low PYQ accuracy (28%)', recommendation: 'Revise Semaphores, Mutex locks, and Classical Sync Problems. Solve 15 PYQs.', color: '#F43F5E' },
  { subject: 'Operating Systems', topic: 'Memory Management (Paging)', score: 41, reason: 'Missed in last mock', recommendation: 'Focus on Paging, Segmentation, TLB, and Page Replacement algorithms.', color: '#F97316' },
  { subject: 'TOC', topic: 'Context-Free Grammars', score: 38, reason: 'Concept gap in derivations', recommendation: 'Practice CFG derivations, parse trees, and ambiguity checking.', color: '#F43F5E' },
  { subject: 'TOC', topic: 'Turing Machines', score: 45, reason: 'Skipped during syllabus', recommendation: 'Watch NPTEL lectures on TM design and practice 10 problems.', color: '#F97316' },
  { subject: 'Computer Networks', topic: 'Congestion Control', score: 48, reason: 'Accuracy drop from 75% to 52%', recommendation: 'Review TCP Reno, Tahoe, AIMD, and Fair Queuing concepts.', color: '#F59E0B' },
  { subject: 'COA', topic: 'Pipeline Hazards', score: 52, reason: 'Confuses data & control hazards', recommendation: 'Draw pipeline diagrams. Practice hazard detection and forwarding.', color: '#F59E0B' },
  { subject: 'DBMS', topic: 'Normalization (BCNF)', score: 55, reason: 'Borderline weak', recommendation: 'Solve decomposition problems. Practice 3NF vs BCNF identification.', color: '#F59E0B' },
  { subject: 'DSA', topic: 'Graph Algorithms', score: 58, reason: 'Slow problem solving', recommendation: 'Practice MST, Dijkstra, Topological Sort — time-box each to 5 min.', color: '#22C55E' },
];

const SUBJECTS = [...new Set(MOCK_WEAK_TOPICS.map(t => t.subject))];

export default function WeakTopicsPage() {
  const [filter, setFilter] = useState('All');

  const filtered = filter === 'All' ? MOCK_WEAK_TOPICS : MOCK_WEAK_TOPICS.filter(t => t.subject === filter);
  const weakCount = filtered.filter(t => t.score < 50).length;
  const avgScore = Math.round(filtered.reduce((s, t) => s + t.score, 0) / filtered.length);
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          {[
            ...
            { label: 'Subjects Affected', value: SUBJECTS.length, color: '#F59E0B' },
            { label: 'Critical (Score < 50)', value: MOCK_WEAK_TOPICS.filter(t => t.score < 50).length, color: '#F97316' },
          ].map((s) => (
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-[9px] px-1.5 py-0.5 rounded-full" style={{ background: `${t.color}15`, color: t.color }}>{t.subject}</span>
