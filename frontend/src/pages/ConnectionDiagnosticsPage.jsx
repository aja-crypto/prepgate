...
export default function ConnectionDiagnosticsPage() {
  const { running, results, progress, startDiagnostics, retryDiagnostics, openDiagnostics } = useDiagnostics();
      ...
      <div className="rounded-2xl p-6 mb-6" style={{ background: 'linear-gradient(180deg, rgba(15,18,32,0.9), rgba(10,14,26,0.9))', border: '1px solid rgba(139,92,246,0.14)', boxShadow: '0 8px 32px rgba(0,0,0,0.25)' }}>
        <div className="flex flex-col md:flex-row items-center gap-6">
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            {running && (
              <div className="mt-3">
                ...
                <div className="text-[11px] text-text3 mt-1">{`${Math.round(progress*100)}%`}</div>
              </div>
            )}
          </div>
          <div className="flex flex-col gap-2 shrink-0 w-full md:w-auto">
            ...
            {results && (results.grade==='poor' || results.grade==='fair') && <button onClick={retryDiagnostics} disabled={running} className="px-6 py-2.5 rounded-xl text-xs font-semibold bg-amber-500/10 border border-amber-500/20 text-amber-400 disabled:opacity-50">Retry Failed Checks</button>}
