...
export default function SubjectsPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (!mountedRef.current) return;
      setError(getApiErrorMessage(err, 'Failed to load syllabus'));
    ...
    <div>
      ...
      {error && !loading && (
        <div className="bg-surface border border-border rounded-xl p-6 text-center">
          <p className="text-sm text-text2 mb-3">{error}</p>
          <button type="button" onClick={load} className="bg-primary text-white px-4 py-2 rounded-lg text-xs font-semibold">Retry</button>
        </div>
      )}
      {!loading && !error && (
      <>
