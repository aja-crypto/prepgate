...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    progressService.getSnapshots().then((res) => { if (mountedRef.current) setSnapshots(res.data?.data || []); }).catch(() => {});
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      // resetAllProgress may throw but we still close the modal
    } finally {
      setResetting(false);
        ...
        <div className="space-y-3">
          ...
          {!user?.isVerified && (
            <button
              ...
              className="text-xs bg-yellow-500/10 border border-yellow-500/20 text-yellow-400 px-3 py-2 rounded-lg disabled:opacity-50"
            >
          ...
          {snapshots.map((s) => (
            <div key={s._id} className="flex items-center justify-between bg-bg-2 border border-border rounded-lg p-3">
              ...
              <button
                ...
                onClick={async () => {
                  setRestoring(s._id);
                  try { await restoreFromSnapshot(s._id); progressService.getSnapshots().then((r) => { if (mountedRef.current) setSnapshots(r.data?.data || []); }); }
                  finally { setRestoring(null); }
                }}
                className="text-[10px] text-primary hover:underline disabled:opacity-50"
              >
                {restoring === s._id ? 'Restoring...' : 'Restore'}
              </button>
            </div>
          ))}
        </div>
      ...
      <Modal open={showDeleteAccount} onClose={() => setShowDeleteAccount(false)} title="⚠️ Delete Account?">
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            disabled={deletingAccount || (!deletePwd && user?.authProvider !== 'google')}
