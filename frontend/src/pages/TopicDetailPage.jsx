...
export default function TopicDetailPage() {
  ...
  if (loading) return (
    <div className="animate-fade-in">
      ...
      <div className="mt-5 space-y-3 animate-pulse">
        ...
        <div className="h-10 bg-surface border border-border rounded-xl" />
  ...
  if (!data) return <div className="text-center py-16"><p className="text-sm text-text3 mb-3">Topic not found</p><Link to="/subjects" className="text-xs text-primary">Back to Subjects →</Link></div>;
    ...
    <div>
      ...
      {tab === 'Practice' && (
        <div className="space-y-4">
          <button type="button" onClick={startTopicMock} disabled={generating} className="btn-primary w-full md:w-auto disabled:opacity-50">{generating ? 'Generating...' : '🎯 Start Topic Mock Test'}</button>
      ...
      {tab === 'Resources' && (
        <div className="space-y-3">
          ...
          {(topic.resources || []).filter((r) => r.type === 'video').map((r, i) => (
            <div key={i} className="flex items-center gap-3 bg-surface border border-border rounded-xl p-4 hover:border-white/15">
              <span className="text-lg">▶</span>
              <a href={r.url} target="_blank" rel="noopener noreferrer" className="text-sm text-text hover:text-primary flex-1">{r.title}</a>
              <button
                type="button"
                onClick={() => { playVideo(r.url, r.title); enterPip(); }}
                className="gx-pip-trigger"
              >
                <svg viewBox="0 0 16 16" fill="currentColor"><path d="M2 3.5A1.5 1.5 0 013.5 2h9A1.5 1.5 0 0114 3.5v7a1.5 1.5 0 01-1.5 1.5h-9A1.5 1.5 0 012 10.5v-7zM4 4v6h8V4H4z"/></svg>
                PiP
              </button>
            </div>
          ))}
