...
import { noteService, resolveAssetUrl } from '../services/api';
...
export default function NotesPage() {
  ...
  const mediaUrl = (note) => resolveAssetUrl(note?.imageUrl || note?.fileUrl);
  const downloadUrl = (note) => resolveAssetUrl(note?.fileUrl || note?.imageUrl);
                    ...
                    >
                      {n.fileType?.includes('pdf') ? (
                        ...
                        <img 
                          ...
                          onError={(e) => {
                            console.error(`Failed to load image for note: ${n.title}`, mediaUrl(n));
                            e.target.src = 'https://placehold.co/600x400/1a1a1a/4f8dff?text=Image+Not+Found';
        ...
        <div className="fixed inset-0 z-[10000] flex flex-col bg-black/95 animate-fade-in">
          <div className="p-4 flex items-center justify-between border-b border-white/10 bg-black/50 backdrop-blur-md">
            ...
            <div className="flex items-center gap-4">
              ...
              <a href={downloadUrl(viewingMedia)} download className="p-2.5 bg-primary text-white rounded-xl hover:opacity-90 transition-all">
          ...
          <div className="flex-1 overflow-auto p-8 flex items-center justify-center custom-scrollbar">
            {viewingMedia.fileType?.includes('pdf') ? (
              <iframe src={mediaUrl(viewingMedia)} className="w-full h-full max-w-5xl rounded-xl shadow-2xl bg-white" title="PDF Viewer" />
            ) : (
              <img 
                src={mediaUrl(viewingMedia)}
