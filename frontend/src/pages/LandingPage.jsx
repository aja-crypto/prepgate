...
const GlobalLivingWallpaper = lazy(() => import('../components/common/GlobalLivingWallpaper'));
const ProductShowcase = lazy(() => import('../components/common/ProductShowcase'));
...
const FEATURES = [
  ...
  { icon: '📚', title: 'Learning Hub', desc: 'Curated videos, roadmaps, community resources, and topic-based learning paths.' },
...
const FEATURE_MARQUEE_ROW_1 = [
  ...
  { icon: '📚', title: 'Learning Hub', description: 'Curated video lectures, subject roadmaps, formula sheets, and organized notes — every resource organized by topic for focused learning.', accent: 'emerald', tagline: 'All in One Place' },
...
export default function LandingPage() {
  ...
  useEffect(() => {
    const idle = window.requestIdleCallback || ((cb, opts) => setTimeout(cb, opts?.timeout || 300));
    const id1 = idle(() => setShowWallpaper(true), { timeout: 1200 });
    const id2 = idle(() => setShowShowcase(true), { timeout: 2000 });
    return () => {
      if (window.cancelIdleCallback) { try { window.cancelIdleCallback(id1); window.cancelIdleCallback(id2); } catch {} }
      else { clearTimeout(id1); clearTimeout(id2); }
    };
  }, []);

  useEffect(() => {
    const apiBase = import.meta.env.VITE_API_URL || 'https://gatenexa-api.onrender.com/api';
    const idle = window.requestIdleCallback || ((cb, opts) => setTimeout(cb, opts?.timeout || 800));
    const id = idle(() => {
    ...
    return () => {
      if (window.cancelIdleCallback) try { window.cancelIdleCallback(id); } catch {}
          ...
          <div className="mobile-carousel">
            {[
              ...
              <div key={i} className="mobile-carousel-card">
                ...
                <div className="text-[9px] text-text3 mt-0.5">GATE 2027 Aspirant</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="relative z-10 px-6 py-16 max-w-6xl mx-auto">
        <AnimatedSection>
          ...
          <div className="flex items-center justify-between mb-8">
            <p className="text-sm text-gray-400">Real questions, real answers — curated from the GATE community.</p>
      ...
      <section className="relative z-10 px-6 py-16 max-w-6xl mx-auto">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {[
            ...
            { title: 'Community Insights', desc: 'Real questions curated from the GATE community', icon: '🔥', link: '/insights', color: '#F472B6' },
                ...
                <div className="relative">
                  ...
                  <div className="space-y-8">
                    {[
                      ...
                      { date: 'AI Integration', desc: 'AI-powered mentor, personalized planner, and intelligent coach chat.' },
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            ...
            <div className="col-span-2 md:col-span-1">
              ...
              <div className="flex gap-3">
                <span className="w-8 h-8 rounded-lg flex items-center justify-center opacity-30 cursor-not-allowed" style={{ border: '1px solid rgba(255,255,255,0.06)' }} aria-label="GitHub" title="Coming soon"><svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 text-slate-400" aria-hidden="true"><path d="M12 0C5.37 0 0 5.37 0 12c0 5.31 3.435 9.795 8.205 11.385.6.105.825-.255.825-.57 0-.285-.015-1.23-.015-2.235-3.015.555-3.795-.735-4.035-1.41-.135-.345-.72-1.41-1.23-1.695-.42-.225-1.02-.78-.015-.795.945-.015 1.62.87 1.845 1.23 1.08 1.815 2.805 1.305 3.495.99.105-.78.42-1.305.765-1.605-2.67-.3-5.46-1.335-5.46-5.925 0-1.305.465-2.385 1.23-3.225-.12-.3-.54-1.53.12-3.18 0 0 1.005-.315 3.3 1.23.96-.27 1.98-.405 3-.405s2.04.135 3 .405c2.295-1.56 3.3-1.23 3.3-1.23.66 1.65.24 2.88.12 3.18.765.84 1.23 1.905 1.23 3.225 0 4.605-2.805 5.625-5.475 5.925.435.375.81 1.095.81 2.22 0 1.605-.015 2.895-.015 3.3 0 .315.225.69.825.57A12.02 12.02 0 0024 12c0-6.63-5.37-12-12-12z"/></svg></span>
                <span className="w-8 h-8 rounded-lg flex items-center justify-center opacity-30 cursor-not-allowed" style={{ border: '1px solid rgba(255,255,255,0.06)' }} aria-label="LinkedIn" title="Coming soon"><svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 text-slate-400" aria-hidden="true"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg></span>
                <span className="w-8 h-8 rounded-lg flex items-center justify-center opacity-30 cursor-not-allowed" style={{ border: '1px solid rgba(255,255,255,0.06)' }} aria-label="X" title="Coming soon"><svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 text-slate-400" aria-hidden="true"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg></span>
                <a href="mailto:hello@gatenexa.app" aria-label="Email us" className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-white/5 transition-all" style={{ border: '1px solid rgba(255,255,255,0.06)' }}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-4 h-4 text-slate-400" aria-hidden="true"><path d="M22 6L12 13 2 6v10a2 2 0 002 2h16a2 2 0 002-2V6z"/><path d="M2 6l10 7 10-7"/></svg></a>
              </div>
            </div>
            {/* Platform */}
