...
export default function PYQPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      refreshPyqs?.(),
      pyqService.getStats().then((r) => { if (mountedRef.current) setGlobalStats(r.data.data); }).catch(() => {}),
      pyqService.getBrowse().then((r) => { if (mountedRef.current) setBrowse(r.data.data); }).catch(() => {}),
  ...
  const toggle = useCallback(async (q, field) => {
    ...
    try {
      ...
      updatePyqs((d) => d.map((item) => (item.id === q.id ? { ...item, [field]: previous } : item)));
