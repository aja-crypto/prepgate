...
export default function AdminPage() {
  ...
  useEffect(() => { mountedRef.current = true; loadAdminData(); return () => { mountedRef.current = false; }; }, [tab]);
    ...
    <div>
      ...
      <Modal open={showSubjectModal} onClose={() => setShowSubjectModal(false)} title={editSubjectId ? 'Edit Subject' : 'Add Subject'}>
        <div className="space-y-3">
          ...
          <button onClick={saveSubject} disabled={savingSubject} className="w-full bg-primary text-white py-2.5 rounded-lg text-sm font-semibold disabled:opacity-50">{savingSubject ? 'Saving...' : editSubjectId ? 'Update' : 'Create'}</button>
        </div>
      </Modal>
    </div>
  );
