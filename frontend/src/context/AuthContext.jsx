...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      authService.getMe()
        .then((res) => { if (!cancelled) setUser(res.data.data.user); })
        .catch(() => {
    ...
    // Safety timeout — if loading never resolves (interceptor edge case), force it
    const safetyTimer = setTimeout(() => { if (!cancelled) setLoading(false); }, 20000);
