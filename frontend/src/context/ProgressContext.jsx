  ...
  useEffect(() => {
    ...
    const handleStorage = (e) => {
      ...
      try {
        ...
        setData((prev) => {
          if (parsed.lastSaved && prev.lastSaved && parsed.lastSaved <= prev.lastSaved) return prev;
          return mergeProgressData(parsed);
        });
    ...
    setData((d) => {
      ...
      if (mongoAvailable) {
        ...
        changed.forEach((item) => {
          const prev = d.pyqs.find((x) => x.id === item.id);
          if (!prev) return;
          if (item.solved !== prev.solved) pyqService.toggleSolved(item.mongoId, item.solved).catch(() => {});
          if (item.bookmarked !== prev.bookmarked) pyqService.toggleBookmark(item.mongoId, item.bookmarked).catch(() => {});
          const flags = {};
          if (item.revisionNeeded !== prev.revisionNeeded) flags.revisionNeeded = item.revisionNeeded;
          if (item.markedDifficult !== prev.markedDifficult) flags.markedDifficult = item.markedDifficult;
          if (Object.keys(flags).length) pyqService.updateFlags(item.mongoId, flags).catch(() => {});
        });
      }
      return { ...d, pyqs: next };
    });
